...
class bodyshop extends MX_Controller {
    ...
    public function __construct()
    {
        ...
        $this->load->model('m_bodyshop', 'mb', TRUE);
        $this->load->library(array('session','blade','form_validation','zip','table','email'));
        $this->load->helper(array('form', 'html', 'companies', 'url','dompdf','date','download','correo'));
    ...
    public function index(){
      ...
      $data['drop_proyectoNombre'] = form_dropdown('proyectoNombre',array_combos($this->principal->get_table_order('bodyshop_tipo','nombre','ASC'),'nombre','nombre',TRUE),"",'class="form-control busqueda" id="proyectoNombre"'); 
    ...
    public function test(){
        ...
        $cuerpo = "";
        $cuerpo .= "Se creo un usuario y contraseña para ver el proceso del proyecto <br> usuario: usuario<br> contraseña: 123<br> Para ingresar de click a este enlace ";
        $cuerpo .= "<a href='".base_url('login/clientes/')."' >Ir a sitio</a>";
        $cuerpo .= "<br>";
        $cuerpo .= "<img src='http://planificadorempresarial.mx/cs/ford_plasencia/statics/img/Firma_BodyShop.png' alt='Firma BodyShop'>";

          
        enviar_correo_general('albertopitava@gmail.com',"Body shop",$cuerpo,array());
    ...
    public function save_proyecto(){
        ...
        $postarray = array(
          ...
          'asesor'=>$post['asesor'],
          'fecha'=>($post['fecha']!='')?$post['fecha']:null,
          'id_horario'=>isset($post['id_horario']) ? $post['id_horario']: null,
          ...
          'transito'=>($post['transito']!='')?$post['transito']:null,
        ...
        $dataCorreo = array('sendTo' => $post['datos_email'], 
          ...
          $cuerpo = '';
          $cuerpo .= "Se creo un usuario y contraseña para ver el proceso del proyecto <br> usuario: ".$dataCorreo['sendTo']."<br> contraseña: ".$dataCorreo['password']."<br> Para ingresar de click a este enlace ";
          $cuerpo .= "<a href='".base_url('login/clientes/')."' >Ir a sitio</a>";
          $cuerpo .= "<br>";
          $cuerpo .= "<img src='http://planificadorempresarial.mx/cs/ford_plasencia/statics/img/Firma_BodyShop.png' alt='Firma BodyShop'>";

          enviar_correo_general($dataCorreo['sendTo'],"Body shop",$cuerpo,array());
          $id = $res;
    ...
    public function asignar_tecnico()
    {
        if ($this->input->post('idproyecto')) {
            ...
            $id_status = $this->principal->getStatus($idproyecto);

            //Insertar en la tabla de los técnicos por estatus por proyecto
            $data = array('proyectoId' => $idproyecto,'status'=>$id_status,'idusuario'=>$this->session->userdata('id'),'fecha'=>date('Y-m-d H:i:s'),'id_tecnico'=>$idtecnicos );
    ...
    public function cambiar_status(){
      ...
      //Si el estatus es igual a armado asignarle por default el técnico que tiene en laminado
      if($status==9){
        $id_laminero = $this->mb->getLamineroStatus($id);
        if($id_laminero!=''){
          $data = array('proyectoId' => $id,'status'=>$status,'idusuario'=>$this->session->userdata('id'),'fecha'=>date('Y-m-d H:i:s'),'id_tecnico'=>$id_laminero );
          $this->db->insert('bodyshop_tecnicos_estatus',$data);
        }
      }
