  ...
  $('.participar').click(function(){
    ...
    $.ajaxData({
      ...
      method: 'post'
        ...
        $('#agregar-participacion').modal('show');
        $('#form-propuesta-colaboracion').off('submit');
        $('#form-agregar-participacion').submit(function(event){
          event.preventDefault();
          //if($.requerido('#prop-propuesta')){
            $.ajaxSerialize($(this),function(data){
              if(data.resp){
                $('#agregar-participacion').modal('hide');
                swal(data.titulo, data.mensaje, "success");
                location.reload();
              }else{
                swal(data.titulo, data.mensaje, "warning");
...
function sendIdParticipar(id){
  $('#idParticipacion').val(id);
}
